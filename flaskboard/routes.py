...
def login():
    ...
    if form.validate_on_submit():
        if form.email.data == 'admin@blog.com' and form.password.data =='password':
            flash('Admin has been logged in','success')
...
def post2(post_id):
    post = Post.query.get_or_404(post_id)
    return render_template('post2.html', title=post.title, post=post)

@app.route("/post2/<int:post_id>/delete", methods=['POST'])
@login_required
def admin_delete_post(post_id):
    post = Post.query.get_or_404(post_id)
    db.session.delete(post)
    db.session.commit()
    flash('Admin has deleted the post!', 'success')
...
def account2():
    ...
    if form.validate_on_submit():
        ...
        form.email.data = current_user.email
    image_file = url_for('static', filename='profile_pics/' + current_user.image_file)
    return render_template('account2.html', title='Account',
                           image_file=image_file, form=form)
